        ...
        if "hint" in request.form:
            if hints_remaining > 0 and attempts > 1:
                ...
                if hint_letter:
                    for i, char in enumerate(selected_word):
                        if char == hint_letter:
                            hidden_word[i] = char  # Reveal all occurrences of the hint letter
                    if hint_letter not in used_letters:
                        used_letters.append(hint_letter)  # Mark the hint letter as used
                    hints_remaining -= 1
                    attempts -= 1
                    message = f"Hint used! '{hint_letter}' revealed. Hints left: {hints_remaining}. Attempts left: {attempts}."
                else:
            ...
            if letter in used_letters:
                ...
                update_hidden_word(letter)  # Update only the revealed parts
