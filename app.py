    ...
    if request.method == "POST":
            ...
            if session['hints_remaining'] > 0 and session['attempts'] > 1:
                for i, char in enumerate(session['selected_word']):
                    if session['hidden_word'][i] == "_":
                        ...
                        session['attempts'] -= 1
                        update_hidden_word()
                        message = f"Hint used! '{char}' revealed. Hints left: {session['hints_remaining']}. Attempts left: {session['attempts']}."
                        break
                else:
        ...
        if "letter" in request.form:
            ...
            if letter in session['used_letters']:
                ...
                session['used_letters'].append(letter)
                message = f"'{letter}' is not in the word. Attempts left: {session['attempts']}."

    if "_" not in session['hidden_word']:
        message = f"Congratulations! You guessed the word: '{session['selected_word']}'! Play again?"
    elif session['attempts'] <= 0:
        message = f"Game Over! The word was '{session['selected_word']}'. Play again?"
